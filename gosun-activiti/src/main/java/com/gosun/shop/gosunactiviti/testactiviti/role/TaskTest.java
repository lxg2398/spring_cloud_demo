package com.gosun.shop.gosunactiviti.testactiviti.role;
...
public class TaskTest {
    ...
    public void deploymentProcessDefinition_zip() {
        //获取输入流
        InputStream in = this.getClass().getClassLoader().getResourceAsStream("diagrams/GroupRole.zip");
        ZipInputStream zipInputStream = new ZipInputStream(in);
        Deployment deployment = processEngine.getRepositoryService()//与流程定义和部署对象相关的Service
                .createDeployment()//创建一个部署对象
                .name("groupRole")//添加部署的名称
                .addZipInputStream(zipInputStream)//
                .deploy();//完成部署
        System.out.println("部署ID：" + deployment.getId());//部署ID：65001
        ...
        /**
         * identityService.newGroup("总经理"),设置的是角色的ID，不是名称
         * identityService.saveUser(identityService.newUser("张三"));设置的是用户的ID，不是名称
         * identityService.createMembership("张三", "部门经理");绑定的是角色ID与用户ID的关系
         */
    }
    ...
    public void startProcessInstance() {
        ...
        System.out.println("流程实例ID:" + pi.getId());//流程实例ID  75001
