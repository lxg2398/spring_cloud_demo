...
public class ServiceTaskTest {
    ...
    public void deployProcessDefinition_2(){
        //获取输入流
        InputStream in = this.getClass().getClassLoader().getResourceAsStream("diagrams/ServiceTask.zip");
        ZipInputStream zipInputStream = new ZipInputStream(in);
        Deployment deploy = processEngine.getRepositoryService()//获取与流程定义和部署对象相关的Service。
                .createDeployment()//创建一个部署对象
                .name("serviceTask")//设置部署名称
    ...
    public void startProcessDefinition(){
         ...
         */
        String key = "serviceTask"; //这个key的值，就是表act_re_procdef中key字段的值，也是testActiviti.bpmn文件中定义的id的属性值。
        Map<String ,Object> map = new HashMap<>();
        map.put("userName","张三");
        ProcessInstance pi = processEngine.getRuntimeService()//获取与正在执行的流程实例和执行对象相关的Service。
                .startProcessInstanceByKey(key,map);//通过流程定义的key 启动实例
